...
local subtitle_data = {
  {"Date", 60, function(_entry, _server_name)
    return date("%m/%d/%y", _entry["date"]) or "" end},
  {"Lvl", 30, function(_entry, _server_name) return _entry["level"] or "" end},
  {"Name", 90, function(_entry, _server_name) return _entry["name"] or "" end},
  {"Class", 60, function(_entry, _server_name)
  ...
  {"Race", 60, function(_entry, _server_name)
    local race_info = C_CreatureInfo.GetRaceInfo(_entry["race_id"]) 
    return race_info.raceName or ""
  end},
  {"Guild", 120, function(_entry, _server_name) return _entry["guild"] or "" end},
  {"Zone/Instance", 100, function(_entry, _server_name) 
    ...
    if map_info then return map_info.name end
    return  "-----------"
  end},
  {"Death Source", 140, function(_entry, _server_name) return id_to_npc[_entry["source_id"]] or "" end},
  {"Last Words", 200, function(_entry, _server_name) return _entry["last_words"] or "" end},
}
...
for idx,v in ipairs(subtitle_data) do 
  ...
  header_strings[v[1]]:SetTextColor(.7,.7,.7)
...
for i=1,100 do
  ...
  row_backgrounds[i]:SetVertexColor(.5, .5, .5, (i%2)/10)
...
local function createDeadliestCreaturesEntry()
  ...
  frame.background:SetVertexColor(.6,.2,.2)
  ...
  frame.creature_name:SetTextColor(.9,.9,.9)
...
local function createMapHighlightButton(name, topleft, scalex, scaley, button_topleft, button_scalex, button_scaley, map_id)
...
for k, class_id in pairs(class_tbl) do
  ...
  for _, v in ipairs(average_class_subtitles) do
    average_class_font_strings[class_id][v[1]] = average_class_container:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    average_class_font_strings[class_id][v[1]]:SetPoint("TOPLEFT", average_class_container, "TOPLEFT", v[2], sep+2)
...
local function setDeathlogMenuLogData(data)
    local ordered = deathlogOrderBy(data, function(t,a,b) return tonumber(t[b]['date']) < tonumber(t[a]['date']) end)
    for idx, v in ipairs(ordered) do
      for _,col in ipairs(subtitle_data) do 
	font_strings[idx][col[1]]:SetText(col[3](v, ""))
      end
      if idx > 99 then break end
    end
    if #ordered == 1 then
...
local function drawLogTab(container)
	...
	local filter = function(server_name, _entry)
	  if name_filter ~= nil then
	    if name_filter(server_name, _entry) == false then return false end
	  end
	  if min_level_filter ~= nil then
	    if min_level_filter(server_name, _entry) == false then return false end
	  end
	  if max_level_filter ~= nil then
	    if max_level_filter(server_name, _entry) == false then return false end
	  end
	  if death_source_filter ~= nil then
	    if death_source_filter(server_name, _entry) == false then return false end
	  end
	  if class_filter ~= nil then
	    if class_filter(server_name, _entry) == false then return false end
	  end
	  if race_filter ~= nil then
	    if race_filter(server_name, _entry) == false then return false end
	  end
	  if zone_filter ~= nil then
	    if zone_filter(server_name, _entry) == false then return false end
	  end
	  if guild_filter ~= nil then
	    if guild_filter(server_name, _entry) == false then return false end
	  end
	  return true
	end
	...
	class_search_box:SetCallback("OnValueChanged", function()
	  ...
	  if key ~= "" then
	    class_filter = function(_, _entry) if class_tbl[key] == tonumber(_entry["class_id"]) then return true else return false end end
	...
	race_search_box:SetCallback("OnValueChanged", function()
	  ...
	  if key ~= "" then
	    race_filter = function(_, _entry) if race_tbl[key] == tonumber(_entry["race_id"]) then return true else return false end end
	    clearDeathlogMenuLogData()
	    setDeathlogMenuLogData(deathlogFilter(_deathlog_data, filter))
	  else
	    race_filter = nil
	...
	zone_search_box:SetCallback("OnValueChanged", function()
	  ...
	  if key ~= "" then
	    zone_filter = function(_, _entry) if zone_tbl[key] == tonumber(_entry["map_id"]) then return true else return false end end
	    clearDeathlogMenuLogData()
	    setDeathlogMenuLogData(deathlogFilter(_deathlog_data, filter))
	  else
	    zone_filter = nil
	...
	player_search_box:SetCallback("OnEnterPressed", function()
	  ...
	  if #text > 0 then 
	    name_filter = function(_, _entry) if string.find(string.lower(_entry["name"]), string.lower(text)) then return true else return false end end
	    clearDeathlogMenuLogData()
	    setDeathlogMenuLogData(deathlogFilter(_deathlog_data, filter))
	  else
	    name_filter = nil
	...
	guild_search_box:SetCallback("OnEnterPressed", function()
	  ...
	  if #text > 0 then 
	    guild_filter = function(_, _entry) if string.find(string.lower(_entry["guild"]), string.lower(text)) then return true else return false end end
	    clearDeathlogMenuLogData()
	    setDeathlogMenuLogData(deathlogFilter(_deathlog_data, filter))
	  else
	    guild_filter = nil
	...
	death_source_box:SetCallback("OnEnterPressed", function()
	  ...
	  if #text > 0 then 
	    death_source_filter = function(_, _entry) if id_to_npc[_entry["source_id"]] ~= nil and string.find(string.lower(id_to_npc[_entry["source_id"]]), string.lower(text)) then return true else return false end end
	    clearDeathlogMenuLogData()
	    setDeathlogMenuLogData(deathlogFilter(_deathlog_data, filter))
	  else
...
local function drawStatisticsTab(container)
	...
	if average_class_container.heading == nil then
	  ...
	  average_class_container.heading:SetTextColor(.9,.9,.9)
	...
	local function updateClassAverages()
	  ...
	  if map_id == 1414 or map_id == 1415 then return end
	  if map_id == 947 then map_id = "all" end
	  if _general_stats["all"][map_id] == nil then return end
	  ...
	  for k,class_id in pairs(class_tbl) do 
		...
		class_str = "all"
	      ...
	      entry_data[class_id]["%"] = string.format("%.1f", v["all"]["num_entries"]/_general_stats["all"][map_id]["all"]["all"]["num_entries"] * 100.0) .. "%"
	...
	if deadliest_creatures_container.heading == nil then
	  deadliest_creatures_container.heading = deadliest_creatures_container:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	  ...
	  deadliest_creatures_container.heading:SetTextColor(.9,.9,.9)
	...
	local function refreshDeadlyUnits()
	  ...
	  if most_deadly_units and #most_deadly_units > 0 then
	    ...
	    deadliest_creatures_container:SetWidth(scroll_frame.frame:GetWidth()*.2)
	    deadliest_creatures_container:SetHeight(scroll_frame.frame:GetWidth()*.4)
	    for i=1,10 do
	      if i <= #most_deadly_units then
		deadliest_creatures_textures[i]:SetWidth(deadliest_creatures_container:GetWidth())
		deadliest_creatures_textures[i]:SetPoint("TOPLEFT", deadliest_creatures_container, "TOPLEFT", 0, -30-i*15)
		deadliest_creatures_textures[i]:SetBackgroundWidth(deadliest_creatures_container:GetWidth()*most_deadly_units[i][2]/max_kills)
	...
	local function updateGraph()
	  ...
	  if LineFrame.heading == nil then
	    ...
	    LineFrame.heading:SetTextColor(.9,.9,.9)
	  ...
	  local function createLine(name, start_coords, end_coords, color, label)
	    ...
	    graph_lines[name].start_point:SetPoint("BOTTOMLEFT", LineFrame, "BOTTOMLEFT", start_coords[1], start_coords[2])
	    ...
	    if color == nil then
	      graph_lines[name]:SetVertexColor(.5, .5, .5, 0.5)
	    else
	      graph_lines[name]:SetVertexColor(color.r, color.g, color.b, .5)
	    end
	    if label then
		...
		graph_lines[name].label = LineFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	      ...
	      graph_lines[name].label:SetTextColor(.7,.7,.7,.7)
	    end
	    graph_lines[name]:Show()
	  end

	  createLine('y_axis', {LineFrame.offsetx,LineFrame.height+LineFrame.offsety}, {LineFrame.offsetx,LineFrame.offsety})
	  createLine('x_axis', {LineFrame.offsetx,LineFrame.offsety}, {LineFrame.offsetx+LineFrame.width,LineFrame.offsety})
	    ...
	    for i=1,60 do for k,v in pairs(class_tbl) do level_num[v][i] = 0 end end
	    ...
	    for servername, entry_tbl in pairs(filtered_by_map) do
	      for _, v in pairs(entry_tbl) do
		ln_std_dev[v['class_id']] = ln_std_dev[v['class_id']] + (log(v["level"]) - ln_mean[v['class_id']])*(log(v["level"]) - ln_mean[v['class_id']])
	      end
	    end
	    ...
	    local function logNormal(x, mean, sigma)
	      return (1/(x*sigma*sqrt(2*3.14)))*exp((-1/2)*((log(x)-mean)/sigma)*((log(x)-mean)/sigma))
	    end
	    for k,v in pairs(class_tbl) do
	    ...
	    for i=1,5 do
	      createLine('y_tick_'..i.."0", {LineFrame.offsetx,(LineFrame.height+LineFrame.offsety)*i/5}, {LineFrame.offsetx+5,(LineFrame.height+LineFrame.offsety)*i/5}, nil, {string.format("%.1f", i/5/LineFrame.zoomy*100.0) .."%", -23, -5})
	    end

	    for i=1,6 do
	      createLine('x_tick_'..i..'0', {LineFrame.offsetx+LineFrame.width*i/6,LineFrame.offsety}, {LineFrame.offsetx+LineFrame.width*i/6,5}, nil, {i.."0", -5,-11})
	    end
		...
		createLine(k..i, {LineFrame.offsetx+(i-2)/60*LineFrame.width,y1*LineFrame.height*LineFrame.zoomy}, {LineFrame.offsetx+(i-1)/60*LineFrame.width,y2*LineFrame.height*LineFrame.zoomy}, RAID_CLASS_COLORS[string.upper(k)])
	...
	local function setMapRegion(map_id)
	  ...
	  map_container:SetHeight(scroll_frame.frame:GetWidth()*.6*3/4)
	  map_container:SetWidth(scroll_frame.frame:GetWidth()*.6)
	  map_container:Show()
	  if map_container.skulls_checkbox == nil then 
	    map_container.skulls_checkbox = CreateFrame("CheckButton", "DeathlogSkullsCheckbox", scroll_frame.frame, "OptionsBaseCheckButtonTemplate")
	    ...
	    map_container.skulls_checkbox_label:SetTextColor(.9,.9,.9)
	  ...
	  if map_container.map_hint == nil then 
	    ...
	    map_container.map_hint:SetTextColor(.8,.8,.8,.8)
	  ...
	  if map_container.heatmap_checkbox == nil then 
	    map_container.heatmap_checkbox = CreateFrame("CheckButton", "DeathlogHeatmapCheckbox", scroll_frame.frame, "ChatConfigCheckButtonTemplate")
	    ...
	    map_container.heatmap_checkbox.label:SetTextColor(.9,.9,.9)
	  ...
	  if map_container.darken_checkbox == nil then 
	    map_container.darken_checkbox = CreateFrame("CheckButton", "DeathlogHeatmapCheckbox", scroll_frame.frame, "ChatConfigCheckButtonTemplate")
	    ...
	    map_container.darken_checkbox.label:SetTextColor(.9,.9,.9)
	  ...
	  local function getOverlayTextures(map_id)
	    local textures = {}
	    if overlay_info[map_id] == nil then return nil end
	  ...
	  for k,v in ipairs(textures) do
		...
		texture:SetPoint("TOPLEFT", map_textures[1], "TOPLEFT", (v['offsetX'] + (256*(k-1)))*map_container:GetWidth()/1000 - 5, - (v['offsetY']+ (256*(j-1)))*map_container:GetWidth()/1000 + 5)
		texture:SetTexture(v['fileDataIDs'][((j-1)*numTexturesWide)+k],nil,nil,"TRILINEAR")
		if map_container.darken_checkbox:GetChecked() then
		  texture:SetVertexColor(.6,.6,1)
		else
	  ...
	  local num_entries = 1
	  local modified_width = map_container:GetWidth()*.98
	  local modified_height = map_container:GetHeight()*.87
	  if map_container.tomb_tex == nil then map_container.tomb_tex = {} end
	  for servername, entry_tbl in pairs(filtered_by_map) do
	    for k,v in pairs(entry_tbl) do
	      if v["map_id"] and v["map_pos"] then
		    if map_container.tomb_tex[num_entries] == nil then
		      ...
		      map_container.tomb_tex[num_entries]:SetTexture("Interface\\TARGETINGFRAME\\UI-TargetingFrame-Skull")
	  ...
	  for k,v in ipairs(map_container.tomb_tex) do
	    if v.map_id == map_id then
	      v:SetPoint('CENTER', map_container, 'TOPLEFT', modified_width*v.coordinates[1], -modified_height*v.coordinates[2])
	  ...
	  if map_container.heatmap == nil then 
		...
		map_container.heatmap[i][j]:SetColorTexture(1.0,.1,.1,0)
		map_container.heatmap[i][j]:SetWidth(7)
		map_container.heatmap[i][j]:SetPoint('CENTER', map_container, 'TOPLEFT', modified_width*i/100, -modified_height*j/100)
		...
		for xi=1,3 do
		  for yj=1,3 do
		    ...
		    if map_container.heatmap[x_in_map] and map_container.heatmap[x_in_map][y_in_map] then
		      map_container.heatmap[x_in_map][y_in_map].intensity = map_container.heatmap[x_in_map][y_in_map].intensity + iv[xi][yj]
		      if map_container.heatmap[x_in_map][y_in_map].intensity > max_intensity then max_intensity = map_container.heatmap[x_in_map][y_in_map].intensity end
		    end
		  end
		end
	      end
	  ...
	  for i=1,100 do
	    for j=1,100 do
	      if map_container.heatmap[i][j].intensity > .01 then
		...
		map_container.heatmap[i][j]:SetColorTexture(1.0,1.1-map_container.heatmap[i][j].intensity*4,.1, alpha)
	...
	map_container:SetScript("OnMouseDown", function(self, button) 
	    ...
	    if button == "LeftButton" then
	      ...
	      if x < map_container:GetLeft() or x > map_container:GetRight() then return end
	      if y < map_container:GetTop()-(modified_height) or y > map_container:GetTop() then return end
	...
	map_container:SetScript("OnUpdate", function() 
	    ...
	    if info ~= nil then
	      local fileDataID, atlasID, texturePercentageX, texturePercentageY, textureX, textureY, scrollChildX, scrollChildY = C_Map.GetMapHighlightInfoAtPosition(map_container.current_map_id, l_x, l_y)
	      if fileDataID and textureX > 0 and textureY > 0 then 
		overlay_highlight:SetTexture(fileDataID)
		overlay_highlight:SetPoint("TOPLEFT", map_textures[1], "TOPLEFT", scrollChildX*modified_width, -scrollChildY*modified_height)
